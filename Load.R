...
#}
  ...
  dt$y <- as.factor(dt$y)
  dt$ni <- ifelse(dt$y == "3" |dt$y == "4" |dt$y == "5" , c("middle"), c("border")) 
  dt$ni <- as.factor(dt$ni)
  initial_y<-dt$y
  dt$y <- NULL
  ...
  idt<-dropBind(idt,model.matrix(~x6,dt))
  idt<-dropBind(idt,model.matrix(~x1,dt))
  idt<-dropBind(idt,model.matrix(~x2,dt))
  idt<-dropBind(idt,model.matrix(~x3,dt))
  idt<-dropBind(idt,model.matrix(~x4,dt))
  idt<-dropBind(idt,model.matrix(~x7,dt))
...
#   sub.6<-subset(dt,y=="6")
  ...
  names5<-getMainVars(idt,0.1, 9835)
  names5<-append(names5,"ni")
  idt<-idt[,(names(idt) %in% names5)]
  ...
  forest_full <- train(ni~., data=train, method="rf", do.trace=10, ntree=100, trControl = fitControl)
